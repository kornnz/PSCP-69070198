...
```text
โจทย์จะให้เลือกหาเส็นทางที่ดีที่สุดในการเดินไปหาเลข 1 ผ่านเลข input ที่ได้รับ ซึ่งเส้นทางที่ดีที่สุดจะเป็นรูปแบบ พีรามิด

Input:
โปรแกรมรับจำนวนเต็มหนึ่งค่า n

Output:
มีหนึ่งบรรทัดระบุจำนวนกำแพงน้อยที่สุดที่คุณจะต้องพัง เพื่อเดินทางจากห้อง N ไปจนถึงห้องหมายเลข 1 ถ้าไม่ต้องพังกำแพงเลย ให้ตอบ 0

Constraints:
Input มีจำนวณ (1 <= N <= 100,000) ซึ่ง ถ้าใช้รูปเลย จะ timeout
```
...
```text
1: รับค่าจำนวณเต็ม
2:  ใช้ loop หาห้อง และชั่นใน พีรามิด
3: ใช้สูตรหา เส้นทางที่น้อยที่สุด
```
...
```text
วิธีสุดท้ายคือหลังจากรับ input แล้วให้ลองหาห้องผ่าน for loop แล้วเมื่อทราบตำแหน่งแล้ว ก็ใช้สูตร เพื่อหาเส้นทางที่เร็วที่สุด
```
